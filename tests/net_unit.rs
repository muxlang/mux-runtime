...
fn tcp_roundtrip() {
    ...
    assert!(unsafe { mux_rc_dec(bind_addr) });
...
fn udp_roundtrip() {
    ...
    assert!(unsafe { mux_rc_dec(a_bind) });
    assert!(unsafe { mux_rc_dec(b_bind) });
...
fn http_request_response_loopback() {
    ...
    assert!(unsafe { mux_rc_dec(resp_val) });
...
fn http_client_against_local_server() {
    ...
    assert!(unsafe { mux_rc_dec(get_res) });
    assert!(unsafe { mux_rc_dec(get_val) });
    ...
    assert!(unsafe { mux_rc_dec(post_res) });
    assert!(unsafe { mux_rc_dec(post_val) });
...
fn http_request_errors() {
    ...
    assert!(unsafe { mux_rc_dec(no_url_val) });
    ...
    assert!(unsafe { mux_rc_dec(refused_val) });
}
...
fn http_response_validation_and_udp_extras() {
    ...
    assert!(unsafe { mux_rc_dec(no_status) });
    ...
    assert!(unsafe { mux_rc_dec(bad_val) });
    ...
    assert!(unsafe { mux_rc_dec(hdr_val) });
