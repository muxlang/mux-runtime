...
fn path_helpers() {
    assert_eq!(
        ok_string(unsafe { mux_io_join(cstr("/a/b").as_ptr(), cstr("c.txt").as_ptr()) }),
        "/a/b/c.txt"
    );
    assert_eq!(
        ok_string(unsafe { mux_io_basename(cstr("/a/b/c.txt").as_ptr()) }),
        "c.txt"
    );
    assert_eq!(
        ok_string(unsafe { mux_io_dirname(cstr("/a/b/c.txt").as_ptr()) }),
        "/a/b"
    );
}
