...
fn open_read_close_file() {
    ...
    let file = unsafe { mux_open_file(path_c.as_ptr()) };
    assert!(!file.is_null());
    let contents_ptr = unsafe { mux_read_file(file) };
    ...
    let ro = unsafe { mux_open_file(path_c.as_ptr()) };
    assert!(!unsafe { mux_write_file(ro, CString::new("x").unwrap().as_ptr()) });
    unsafe { mux_close_file(ro) };
...
fn value_from_string_and_prints() {
    ...
    let v = unsafe { mux_value_from_string(s.as_ptr()) };
    assert!(!v.is_null());
    // print helpers just need to run without crashing
    unsafe { mux_print(v) };
    unsafe { mux_print_cstr(s.as_ptr()) };
    mux_flush_stdout();
    assert!(unsafe { mux_rc_dec(v) });

    assert!(unsafe { mux_value_from_string(std::ptr::null()) }.is_null());
    unsafe { mux_print_cstr(std::ptr::null()) };
}
